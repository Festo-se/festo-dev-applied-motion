...
class TestFPosAxisMove:
    def test_move_sends_set_par_before_mov_axis(self, fposapi_axis_mock):
        """SET_PAR 103 must be sent before MOV_AXIS so speed is applied."""
        ...
        mov_axis_idx = commands.index("MOV_AXIS")
        assert set_par_idx < mov_axis_idx
    ...
    def test_move_absolute_uses_rel_flag_zero(self, fposapi_axis_mock):
        ...
        mov_axis_call = fposapi_axis_mock._client.send_command.call_args_list[1]
        # args: ("MOV_AXIS", axis_index, rel_flag, position)
        assert mov_axis_call[0][2] == 0
...
class TestFPosAxisStatus:
    ...
    def test_ready_for_motion_always_true(self, fposapi_axis_mock):
